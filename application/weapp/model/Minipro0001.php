...
class Minipro0001 extends Model
{
    ...
    private function get_httpimgurl($content = '')
    {
        ...
        $content = preg_replace($pregRule, '<img ${1} src="'.SITE_URL.'/${4}" ${5} />', $content);
    ...
    public function getImgRealpath($filename, $domain = true)
    {
        ...
        $filename = $web_cmspath.'/weapp/'.$this->nid.'/template/skin/images/'.$filename;
        if ($domain) {
